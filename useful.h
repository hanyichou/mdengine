...
public:
	...
	HOST DEVICE inline explicit Vector3(float s):x(s), y(s), z(s), w(s) {}
	...
	HOST DEVICE inline explicit Vector3(const float* d) : x(d[0]), y(d[1]), z(d[2]), w(0) {}
             DEVICE inline explicit Vector3(const float4 a) : x(a.x ), y(a.y ), z(a.z ), w(a.w) {}
	...
	HOST DEVICE inline void print() const {
        ...
        friend std::istream& operator >> (std::istream &in, Vector3& c)
        {
            in >> c.x >> c.y >> c.z;
            return in;
        }
...
public:
	HOST DEVICE inline Matrix3() : exx(0.), exy(0.), exz(0.), eyx(0.), eyy(0.), eyz(0.), ezx(0.), ezy(0.), ezz(0.), isDiag(0){}
